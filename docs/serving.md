...
encoded inside a bounded window whose device memory is borrowed from the evict-ranked tail of the
text weights (lm_head, token embedding, draft head, MTP head). The window streams the tower through
two layer slots, lands the embeddings in pinned memory, and restores the borrowed tail from a pinned
mirror before the prefill unit ends; the prefill then uploads only each chunk's embedding columns.
Windows are exclusive by construction (the engine runs one GPU unit at a time) and one image wide,
so other requests interleave between images. A follow-up turn over the same image reuses the prefix
and opens no window. Embeddings are produced by the same kernels on the same bytes, so completions
are identical between residencies. `--vision-max-merged N` bounds one item's merged tokens (media
above it is downscaled at preprocessing) and sizes the window. The request log line reports
`overlay=<windows>x<ms> (evict <MiB> <ms>, restore <ms>, staged <MiB>)` and the JSON record carries
`vision_overlay`.
