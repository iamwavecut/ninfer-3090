...
artifact="${repo_dir}/out/qwen3_8_27b_gsq_rco_iq3_s.ninfer"
config="${repo_dir}/eval/configs/qwen3_8_27b_gsq_rco_reasoning.yaml"
...
step_config() {
    case "$1" in
        text)
            # The GSQ-RCO IQ3_S artifact leaves more memory free than the
            # groupwise-int one, so the full 262,144-token context fits.
...
run_step() {
    ...
    server_log="${log_dir}/qwen3_8_27b_gsq_rco-${step}-c4-${run_stamp}.server.log"
    request_log="${log_dir}/qwen3_8_27b_gsq_rco-${step}-c4-${run_stamp}.requests.jsonl"
