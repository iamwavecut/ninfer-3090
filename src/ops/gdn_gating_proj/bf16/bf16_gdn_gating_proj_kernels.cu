...
constexpr std::int32_t cooperative_resident_ctas_per_sm() noexcept {
    ...
    if constexpr (std::is_same_v<Geometry, Bf16Gdn27Geometry>) {
        ...
        // Measured on the sm_86 build (cuobjdump -res-usage): every split runs 8 warps at 65
        // registers, so the 40 KiB of dynamic shared memory is what binds -- two CTAs per SM.
        return 2;
#else
...
#if defined(NINFER_SM8X_COMPAT)
        // Measured on the sm_86 build (cuobjdump -res-usage, 8 warps): split-32 uses 122-126
        // registers and is register bound to two CTAs per SM; split-16 uses 56 registers and
        // reaches the 24-KiB shared-memory bound of four; split-8/4/2 use 74 registers and are
        // register bound to three. Overshooting is not merely slow: the driver rejects the launch
        // with cudaErrorCooperativeLaunchTooLarge.
...
bool bf16_gdn_gating_proj_mma_split4_launch(Bf16GdnGatingTokenVariant variant, const Tensor& x,
    ...
    // threads' worth of registers, which admits a single CTA per sm_86 SM and halves the
    // cooperative budget the launcher can assume.
