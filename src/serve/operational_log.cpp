        ...
        out << " | overlay " << metrics.overlay_windows << 'x'
            << (metrics.overlay_exclusive_windows == metrics.overlay_windows ? "excl"
                : metrics.overlay_exclusive_windows == 0                     ? "conc"
                                                                            : "mixed")
            << ' ' << product::format_pretty_duration(metrics.overlay_window_seconds) << " (evict "
            << product::format_pretty_bytes(metrics.overlay_evicted_bytes) << ' '
            << product::format_pretty_duration(metrics.overlay_evict_seconds) << ", restore "
            << product::format_pretty_duration(metrics.overlay_restore_seconds) << ", staged "
            << product::format_pretty_bytes(metrics.overlay_staged_bytes) << ')';
    }
...
void OperationalLog::device_ecc_warning(int device) const {
    // Inline ECC on GDDR6X GeForce cards reserves ~6.25% of VRAM for checksums and taxes memory
    // bandwidth on every access. Decode is bandwidth-bound, so an ECC-enabled card silently loses
    // a large share of its published throughput and KV capacity while looking exactly like an
    // engine regression. ECC is off by default on GeForce; warn loudly when someone left it on.
    cudaDeviceProp props{};
    if (cudaGetDeviceProperties(&props, device) != cudaSuccess || props.ECCEnabled == 0) { return; }
    write({.severity = OperationalSeverity::Warning,
           .message  = std::string("ECC is ENABLED on ") + props.name +
                      ": GDDR6X stores ECC checksums in VRAM, costing ~6.25% of capacity "
                      "(23,028 vs 24,564 MiB on a 24 GB card) and ~12% of memory bandwidth "
                      "(measured 803 vs 902 GB/s on an RTX 3090 Ti). KV capacity and prefill "
                      "suffer accordingly. ECC is off by default on GeForce; if this is not a "
                      "deliberate reliability choice, disable it with `nvidia-smi -e 0` and "
                      "reboot."});
}
